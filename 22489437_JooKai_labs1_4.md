...
3. We can retrieve similar information using Python3 by running the following commands. This returns and prints the same data as before. However, this data is not tabulated.
```
python3
>>> import boto3
...
3. The following command is used to authorise inbound traffic for ssh: `aws ec2 authorize-security-group-ingress --group-name <student number>-sg --protocol tcp --port 22 --cidr 0.0.0.0/0`. Some things to note about this command:
...
4. The command `aws ec2 create-key-pair --key-name <student number>-key --query 'KeyMaterial' --output text > <student number>-key.pem` creates a key pair that will allow a user to ssh to the EC2 instance. The private key is stored in the output file `22489437-key.pem`
![Creating private key](https://github.com/Bsubs/CITS5503-Cloud-Computing/blob/main/lab2/Screenshot%202023-08-04%20165022.jpg)


5. The private key file is moved into the SSH directory and its permissions are changed to be read only for the owner of the file, and no permissions for others.
![changing permissions](https://github.com/Bsubs/CITS5503-Cloud-Computing/blob/main/lab2/Screenshot%202023-08-04%20165358.jpg)


6. The command ` aws ec2 run-instances --image-id ami-d38a4ab1 --security-group-ids <student number>-sg --count 1 --instance-type t2.micro --key-name <student number>-key --query 'Instances[0].InstanceId' is used to create the EC2 instance. The command returns the instance ID of the created instance.
...
7. Using the instance ID from step 5, a tag is added to the instance.
![create tag](https://github.com/Bsubs/CITS5503-Cloud-Computing/blob/main/lab2/Screenshot%202023-08-04%20165508.jpg)


8. The public IP address of this instance is determined using the command `aws ec2 describe-instances --instance-ids i-<instance id from above> --query 'Reservations[0].Instances[0].PublicIpAddress'`
![public IP](https://github.com/Bsubs/CITS5503-Cloud-Computing/blob/main/lab2/Screenshot%202023-08-04%20165711.jpg)


9. Using the private key file from step 3 and the IP address from step 7, the user is able to SSH into the EC2 instance.
![ssh](https://github.com/Bsubs/CITS5503-Cloud-Computing/blob/main/lab2/Screenshot%202023-08-04%20165859.jpg)


10. The created instance can be viewed from the AWS console
![AWS console](https://github.com/Bsubs/CITS5503-Cloud-Computing/blob/main/lab2/Screenshot%202023-08-04%20170050.jpg)


11. The instance can be terminated from the awscli using the following command: `aws ec2 terminate-instances --instance-ids i-<your instance id>`
...
3. Enabling docker and checking the version installed:
...
6. Building the docker image from the dockerfile and index.html
...
7. Creating a table with the following attributes:
        ```
            CloudFiles = {
                    'userId',
                    'fileName',
